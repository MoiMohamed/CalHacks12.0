...
from asyncpg import NotNullViolationError, UniqueViolationError  # type: ignore[import-untyped]
from pydantic import BaseModel
from sqlalchemy import Delete, Select, Update, delete, insert, select, update  # , text
...
from app.models import *  # noqa: F401, F403, W0401
from app.models.base import DBModel

# from aralects.utils.utils import get_sqlite_dsn_async
...
SessionCreator = async_sessionmaker(bind=engine, autoflush=False, expire_on_commit=False, autocommit=False)
...
async def managed_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    This context manager is intended for use outside of FastAPI's dependency
    injection system, for example, in background tasks or scripts.
    ...
    Usage:
        async with managed_session() as session:
            ...
            # Changes will be committed automatically on successful exit,
...
# Note:
# `get_session` is designed as a FastAPI dependency using `yield`.
# You cannot use `async with get_session() as session:` directly.
# For a context manager pattern outside FastAPI, use `managed_session` above,
# or use the session maker directly:
#
# async with SessionCreator() as session:
#     # ... operations ...
#     await session.commit() # Explicit commit often needed here
#
# `managed_session` provides automatic commit/rollback similar to `get_session`.
